#!/usr/bin/env python3
"""Freeze a target-free absolute S1 LOO threshold on local DL3DV scenes."""
...
from saes.adaptive_l1_calibration import build_calibration_record
...
def _candidate_l1_residuals(plan: Any) -> list[float]:
    ...
    for record in plan.tile_trace:
        is_candidate = record["pre_guard_route"] == "L1" or (
            record["pre_guard_route"] == "L0" and record["depth_uniform"] is True
        )
        if is_candidate:
            values.append(float(record["adaptive_l1_leave_one_out_residual"]))
    if not values:
        raise RuntimeError("adaptive L1 calibration sample has no candidate tiles")
    return values


def collect_calibration_record(
    *, device: torch.device, sample_indices: tuple[int, ...]
) -> dict[str, Any]:
    if (
        ...
        raise ValueError("calibration sample indices must be distinct and exclude sample-0")
    ...
    for sample_index in sample_indices:
        ...
        if context["image"].shape[0] != 1:
            raise RuntimeError("adaptive L1 calibration requires batch size one")
...
def main() -> int:
    ...
    if device.type != "cuda" or not torch.cuda.is_available():
        parser.error("adaptive L1 calibration requires CUDA")
    try:
        record = collect_calibration_record(
            device=device, sample_indices=tuple(args.sample_indices)
        )
