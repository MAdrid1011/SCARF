...
def _require_classic_checkpoint(model: str) -> None:
    checkpoint = ROOT / model / "checkpoints" / "re10k.ckpt"
    if not checkpoint.is_file():
        pytest.skip(
            f"{model} frozen-identity test requires the external Re10K checkpoint"
        )
...
def test_frozen_classic_backend_identity_binds_live_source_files(model: str):
    ...
    _require_classic_checkpoint(model)
    contract = resolve_classic_backend_contract(model, ROOT)
