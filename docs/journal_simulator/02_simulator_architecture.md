...
功能前端在 GPU 上维护中间张量。RMCF 探针统计、FSDR LSH 签名与候选筛选、SAES 探针统计、逐视图质量指标和大规模追踪归约均使用批量 GPU 运算。热路径中禁止对 tile、像素、候选或 Gaussian 执行 `.item()`、`.cpu()`、隐式 Python 标量转换或显式设备同步。
...
周期后端使用紧凑数组保存描述符、依赖计数、资源可用时间和 FIFO 状态，不为每个像素或任务创建 Python 对象。同一周期、同一资源类型且具有相同发射规则的事件可批量计算，但不得合并其地址、依赖、边界 tile 或阻塞状态。

互相独立的场景、视图和消融组合使用多进程或批量 GPU 并行。单个任务图中的严格因果事件可留在 CPU，但应使用编译后的数组内核或等价开源实现。候选统计、trace 分析、质量计算和其他可形成大批次的任务使用 GPU。不得在单核 Python 循环中执行可直接并行的长时间工作。

性能剖析必须分开模拟周期和模拟器自身运行时间。提高实现速度不得通过阶段平均、抽样任务或预设命中率改变周期语义。
...
每次运行保存官方仓库提交、权重发布标识与字节数、数据集样本清单、配置来源图、软件环境、GPU 型号和模拟器提交。追踪文件采用列式分块，允许 GPU 直接统计候选分布、区域形状、Gaussian 数量和地址跨度。Python 对象追踪只允许用于小规模调试。
