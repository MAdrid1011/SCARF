...
class CurrentModuleBehaviorTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "select L0 when feature variance is below threshold" in {
    test(new SAESController) { dut =>
      ...
      dut.io.routeValid.poke(false.B)
      dut.io.routeLevel.poke(SAESLevel.sFull)
      dut.io.l0MaterializationValid.poke(true.B)
      dut.io.l1MaterializationValid.poke(true.B)
  ...
  it should "attempt L1 after an L0 materialization rejection" in {
    test(new SAESController) { dut =>
      dut.io.config.numDepthCandidates.poke(64.U)
      dut.io.config.featureDim.poke(128.U)
      dut.io.config.imageH.poke(16.U)
  ...
  it should "fall closed to Full after an L1 materialization rejection" in {
    test(new SAESController) { dut =>
      dut.io.config.numDepthCandidates.poke(64.U)
      dut.io.config.featureDim.poke(128.U)
      dut.io.config.imageH.poke(16.U)
