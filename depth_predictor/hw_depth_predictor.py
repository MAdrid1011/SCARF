    ...
    def _hw_attention_block(
        ...
        if postnorm:
            ...
            if hasattr(block, 'qkv'):
                ...
                total_cycles += gemm_c.total_cycles if gemm_c else qkv_flat.numel() * C // 256
        ...
        if hasattr(self, '_hw_gemm_unit'):
            ...
            total_cycles += bs * num_heads * length * length * ch // 256
...
class HWDepthPredictor:
    ...
    def _forward_stereo_batched_hw(
        ...
        **kwargs,
        ...
        if _use_orig_cv:
            ...
            cost_volume_cycles = V * B * D * C * H * W * 4  # warping + correlation
    ...
    def _hw_multi_scale_deformable_attn(
        ...
        total_cycles += sampling_locations.numel()  # Multiply-add per element
        ...
        # Cycles for weighted sum: multiply + add per element
        total_cycles += bs * num_heads * embed_dims * num_queries * num_levels * num_points * 2
    ...
    def _hw_uv_coarse_attention(
        ...
        total_cycles += VB * N * num_depth * embed_dims  # Mean cycles
        ...
        total_cycles += VB * N * num_depth * embed_dims  # Multiply cycles
        
        # ====== Step 7: Sum and normalize ======
        # output = sum(-1) / sqrt(C)
        output = output.sum(-1) / math.sqrt(embed_dims)  # [VB, N, D]
        total_cycles += VB * N * num_depth * embed_dims  # Sum cycles
        total_cycles += VB * N * num_depth  # Division cycles
    ...
    def _hw_uv_transformer(
        ...
        for layer_idx, layer in enumerate(encoder.layers):
            ...
            if mode == 'coarse':
                ...
                total_cycles += VB * num_cams * N * num_depth * C // num_cams
                
                # CRITICAL: Multiply with key (features)
                # In original code, key is NOT flipped (only value is flipped)
                # key = key.permute(2, 0, 1, 3).reshape(bsv, num_value, embed_dims)
                # Then: key = key.unsqueeze(2); output = output * key
                # Use 'key' (not flipped) for this multiplication
                key_for_mul = key.unsqueeze(2)  # [VB, N, 1, C]
                output = output * key_for_mul  # [VB, N, D, C]
                total_cycles += VB * N * num_depth * C
                
                # Sum over embedding dimension with scaling
                # Original: output = output.sum(-1) / (embed_dims ** 0.5)
                output = output.sum(-1) / (C ** 0.5)  # [VB, N, D]
                total_cycles += VB * N * num_depth * C
                
                # IMPORTANT: In DepthPredictorTrans, embed_dims=128 and num_depth=128
                # So output [VB, N, D=128] has same last dim as identity [VB, N, C=128]
                # Can directly add as residual
    ...
    def _hw_corr_refine_net(
        ...
        if dp is not None and hasattr(dp, 'corr_refine_net'):
            ...
            total_cycles = VB * (D + C) * D * 9 * H * W * 10 // 256
    ...
    def _hw_depth_head_lowres(
        ...
        if dp is not None and hasattr(dp, 'depth_head_lowres'):
            ...
            # Fallback
            out = x
            total_cycles = VB * D * D * 9 * H * W * 2 // 256
    ...
    def _hw_gaussian_head(
        ...
        if dp is not None and hasattr(dp, 'upsampler'):
            ...
            total_cycles += VB * C * 2 * H_out * W_out
        ...
        if dp is not None and hasattr(dp, 'proj_feature'):
            ...
            total_cycles += VB * C * 64 * 9 * H_out * W_out // 256
        ...
        if dp is not None and hasattr(dp, 'refine_unet'):
            ...
            total_cycles += VB * refine_in_ch * 64 * 9 * H_out * W_out // 256
        ...
        if dp is not None and hasattr(dp, 'to_gaussians'):
            ...
            total_cycles += VB * gau_in_ch * gaussian_raw_channels * 9 * H_out * W_out * 2 // 256
    ...
    def _forward_depthsplat_hw(
        ...
        **kwargs,
        ...
        total_cycles += original_images.numel() * 2  # sub + div
        ...
        total_cycles += features_cnn_pos.numel() * 2
        ...
        for scale_idx in range(num_scales):
            ...
            total_cycles += b_new * num_tgt * c_feat * num_d * feat_h * feat_w
            ...
            total_cycles += depth.numel() * num_d
        ...
        cycle_breakdown = CycleBreakdown(
            cost_volume=total_cycles // 4,
            unet_refinement=total_cycles // 4,
            depth_head=total_cycles // 8,
            softmax_regression=total_cycles // 8,
            upsampling=total_cycles // 4,
        )
    ...
    def _hw_ds_warp_features(
        ...
        with torch.no_grad():
            ...
            total_cycles += b_w * 3 * 3 * h_w * w_w // 256
            ...
            total_cycles += b_w * 3 * 3 * d_w * h_w * w_w // 256
    ...
    def _forward_stereo_perview_hw(
        ...
        **kwargs,
        ...
        for v in range(V):
            ...
            unet_cycles += B * unet_input.shape[1] * D * 9 * H * W * 5 // 256
        ...
        if hasattr(self, '_original_to_gaussians') and self._original_to_gaussians is not None:
            ...
            for layer in to_g:
                if hasattr(layer, 'weight'):
                    ...
                    total_cycles += B * in_ch * out_ch * kH * kW * H_out * W_out // 256
    ...
    def _estimate_cost_volume_cycles(self, B, V, C, H, W, D):
        ...
        # Transformer-based matching: QKV + attention + FFN
        embed_dim = 128
        # Coarse (1 layer) + Fine (2 layers)
        cycles = V * B * 3 * (
    ...
    def _estimate_unet_cycles(self, B, V, C, H, W, D):
        ...
        # Cost volume U-Net
        channels = 128
        cycles = V * B * channels * channels * 9 * H * W // 256
        return cycles
    ...
    def _estimate_regression_cycles(self, B, V, D, H_out, W_out):
        ...
        softmax_cycles = V * B * 3 * D * H_out * W_out // 16  # At output resolution
        regression_cycles = V * B * 2 * D * H_out * W_out // 16
